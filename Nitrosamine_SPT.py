...
def get_scinopharm_apis_auto():
    ...
    try:
        r = requests.get(target_url, headers=REAL_HEADERS, verify=False, timeout=15)
        ...
        for a in soup.find_all('a', href=True):
            if any(k in a.get_text(strip=True) for k in target_keywords):
                ...
                if not full_link.startswith("http"):
                    full_link = base_url + full_link if full_link.startswith("/") else base_url + "/" + full_link
        ...
        for link in pdf_links:
            ...
            try:
                pdf_resp = requests.get(link, headers=REAL_HEADERS, verify=False, timeout=15)
...
def parse_uploaded_file(uploaded_file):
    ...
    try:
        ...
        target_col = None
        target_col_2 = None 
        possible_names = ['product', 'api', 'name', 'drug', 'item', 'substance', '產品', '藥名', '品項']
        ...
        if target_col and "product" in str(target_col).lower():
            for col in df.columns:
                if str(col) != str(target_col) and "product" in str(col).lower() and ("1" in str(col) or "2" in str(col)):
                    target_col_2 = col
                    break
        ...
        for _, row in df.iterrows():
            ...
            if target_col_2:
                ...
                if pd.notna(val2) and str(val2).strip() != '' and str(val2).strip().lower() != 'nan':
...
def is_valid_api_name(text):
    ...
    text = text.lower()
    ignore = ["api name", "regulatory", "therapeutic", "page", "scinopharm", "download", "date", "status", "product"]
...
def get_fda_data():
    ...
    try:
        ...
        if matches:
            logs.append(f"Strategy 1 (JSON Regex): Found {len(matches)} potential JSON data blocks.")
        ...
        for i, table in enumerate(tables):
            try:
                ...
                if not headers_list:
                    ...
                    if first_row:
                        headers_list = [td.get_text(strip=True) for td in first_row.find_all(['td', 'th'])]
                
                # 防呆：確保 headers 不重複
                if headers_list:
                    headers_list = [h if h else f"Unnamed_{j}" for j, h in enumerate(headers_list)]
                    seen = set()
                    new_headers = []
                ...
                if headers_list and rows_data:
                    ...
                    all_tables_data.append(df)
                    logs.append(f"HTML Table {i} parsed successfully with {len(df)} rows.")
            except Exception as e:
        ...
        for df in all_tables_data:
            ...
            for col in df.columns:
                ...
                elif any(k in c_lower for k in ['source', 'drug', 'product', 'api']):
            ...
            if has_critical_data:
                ...
                for req_col in ['Nitrosamine', 'Limit', 'Source', 'Notes', 'IUPAC']:
...
def get_ema_data():
    ...
    try:
        ...
        if target_link:
            ...
            file_resp = requests.get(target_link, headers=headers, verify=False)
            
            temp_df = pd.read_excel(io.BytesIO(file_resp.content), header=None, nrows=30)
            
            best_idx = 0
            max_score = 0
            keywords = ["nitrosamine", "limit", "intake", "substance", "ng/day", "iupac", "impurity", "structure", "cas", "source", "ai (ng/day)"]
            ...
            log_messages.append(f"Header Scoring: Selected Row {best_idx} with score {max_score}")
...
def smart_match(scino_api, row_series):
    ...
    core_tokens = {t for t in scino_tokens if t not in STOP_WORDS and len(t) > 2}
    ...
    row_text = " ".join([str(val).upper() for val in row_series.values if pd.notna(val)])
...
if ready_to_run:
    st.subheader(f"目前監控清單: {len(api_list)} 項產品 ({'自動爬取' if source_mode.startswith('🌐') else '手動匯入'})")
    ...
    if st.button("🚀 開始執行比對 (Start Analysis)", type="primary"):
        ...
        if not fda_df.empty:
            status_box.write(f"✅ FDA: {len(fda_df)} 筆 (已過濾僅 Table 1 & 2), EMA: {len(ema_df)} 筆")
        else:
        ...
        if match_results:
            ...
            # 調整欄位順序
            cols_order = ["Source", "ScinoPharm Product", "SPT Project num", "Nitrosamine Impurity", "IUPAC Name", "Limit (AI)", "Notes", "Reference Value"]

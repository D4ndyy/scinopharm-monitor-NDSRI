...
st.markdown("""
### 🛠️ v7.3 功能更新：
1.  **精準鎖定 FDA Table 1 & 2**：在成功抓取網頁資料後，僅保留前兩個主要表格，排除其他次要資訊。
2.  **JSON 嗅探 (Fallback)**：保留強大的 JSON 解析能力，以應對 FDA 網頁結構變化。
3.  **誤判修正**：Compound/Form 等關鍵字過濾邏輯全保留。
""")
...
STOP_WORDS = {
    ...
    "UNKNOWN", "AND", "WITH", "FORM", "TYPE", "CLASS", "GRADE", "GROUP",
    "PART", "COMPOUND"
}
